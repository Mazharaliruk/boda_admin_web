  ...
  Future<void> updateTransaction(TransactionModel data) async {
    ...
    try {
      ...
      switch (response.statusCode) {
        ...
        case 403:
          throw ForbiddenException(
              'Forbidden: You do not have permission to update this Transaction.');
        case 404:
...
// Delete Transaction
  ...
  Future<TransactionModel?> fetchTransactionByOrder(int orderId) async {
    final String url = '${ApiUrls.fetchTransactionsByOrder}?order=$orderId';
      ...
      switch (response.statusCode) {
        case 200:
          ...
          if (data is List && data.isNotEmpty) {
            // Assume we take the first item in the list as the single transaction
            final firstItem = data.first;
            if (firstItem is Map<String, dynamic>) {
              return TransactionModel.fromMap(firstItem);
            } else {
              throw FormatException(
                  'Invalid data format for the first transaction: $firstItem');
            }
