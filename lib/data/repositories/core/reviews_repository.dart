...
// Method to fetch Orders
  Future<List<ReviewsModel>> fetchReviews() async {
    const String url = ApiUrls.reviews;
      ...
      switch (response.statusCode) {
        case 200:
          ...
          if (data is List) {
            ...
            return data.map<ReviewsModel>((item) {
              if (item is Map<String, dynamic>) {
                print(item);
                return ReviewsModel.fromMap(item);
              } else {
  ...
  final String url = '${ApiUrls.reviews}$id/';
  ...
  final String url = '${ApiUrls.reviewsByEvent}?event=$eventId';
...
// Update Order
  Future<void> updateReviews(EventModel data) async {
    final url = '${ApiUrls.reviews}/${data.id}';
    ...
    try {
      ...
      switch (response.statusCode) {
        ...
        case 403:
          throw ForbiddenException(
              'Forbidden: You do not have permission to update this Orders.');
        case 404:
  ...
  Future<void> deleteReviews(int id) async {
    ...
    try {
      ...
      switch (response.statusCode) {
        ...
        case 403:
          throw ForbiddenException(
              'Forbidden: You do not have permission to delete this Orders.');
        case 404:
          throw NotFoundException('Not Found: The Orders does not exist.');
        case 500:
        case 502:
